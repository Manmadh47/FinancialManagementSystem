...
export class GenerateForecastComponent implements OnInit {
  ...
  fromMonth(monthIndex) {
    ...
    this.endMonths = this.futureMonths.slice(monthIndex);
  }
  ...
  upload(){
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result==true){
        this.forecastService.postRecords(this.finalMonthData).subscribe();
        console.log(result);
      }
    });
  }
  columnBind(forecastMonth) {
    ...
    for (var [key, monthValue] of this.monthMap.entries()) {
      if (key == forecastMonth.toLowerCase()) {
        ...
        this.selctedMonthDate = new Date(dateString);
